...
for player in ivalues(GAMESTATE:GetHumanPlayers()) do
	...
	local Marvelous = string.format("% 4d",STATSMAN:GetCurStageStats():GetPlayerStageStats(player):GetTapNoteScores("TapNoteScore_W1"));
	local Perfect = string.format("% 4d",STATSMAN:GetCurStageStats():GetPlayerStageStats(player):GetTapNoteScores("TapNoteScore_W2"));
	local Great = string.format("% 4d",STATSMAN:GetCurStageStats():GetPlayerStageStats(player):GetTapNoteScores("TapNoteScore_W3"));
	local Good = string.format("% 4d",STATSMAN:GetCurStageStats():GetPlayerStageStats(player):GetTapNoteScores("TapNoteScore_W4"));
	local Almost = string.format("% 4d",STATSMAN:GetCurStageStats():GetPlayerStageStats(player):GetTapNoteScores("TapNoteScore_W5"));
	local Ok = string.format("% 4d",STATSMAN:GetCurStageStats():GetPlayerStageStats(player):GetHoldNoteScores("HoldNoteScore_Held"));
	local Ng = string.format("% 4d",STATSMAN:GetCurStageStats():GetPlayerStageStats(player):GetHoldNoteScores("HoldNoteScore_LetGo"));
	local Boo = string.format("% 4d",STATSMAN:GetCurStageStats():GetPlayerStageStats(player):GetTapNoteScores("TapNoteScore_Miss"));
	...
	t[#t+1] = Def.ActorFrame{
		...
		Def.RollingNumbers {
			...
			SetCommand = function (self)
				...
				self:targetnumber(STATSMAN:GetCurStageStats():GetPlayerStageStats(player):GetTapNoteScores("TapNoteScore_W1"));
			end;
			Condition=Mfix==0
		};
		LoadFont("_resultLbl")..{
		...
		Def.RollingNumbers {
			...
			Condition=not STATSMAN:GetCurStageStats():GetPlayerStageStats(player):FullCombo()
		};
		...
		Def.RollingNumbers {
			...
			Condition=STATSMAN:GetCurStageStats():GetPlayerStageStats(player):FullCombo()
		};
		...
		--Scores
		Def.ActorFrame{
			InitCommand=cmd(x,-132;y,261);
			OnCommand=cmd(sleep,2.433;linear,.433;addy,-40);
		...
		Def.ActorFrame{
			InitCommand=cmd(x,-132;y,319);
			OnCommand=cmd(sleep,2.433;linear,.433;addy,-60);
			LoadActor(THEME:GetPathG("ScreenGameplay", "ScoreFrame/normal"));
			...
			Def.RollingNumbers{
				...
				InitCommand=cmd(x,108;horizalign,right;sleep,2.933;queuecommand,"Set");
				SetCommand = function(self)
					self:Load("RollingNumbersSC4");
					self:targetnumber(ScTotal);
				end;
			};
		};
	};
